...
top = gridspec.GridSpecFromSubplotSpec(1, 
                                       ...
                                       wspace=0.275)
ax = [fig.add_subplot(top[0,i]) for i in range(3)]
...
params = ['w', 'w0pwa', 'As_1e9', 'ns', 'omegam', 'H0']
latex  = ["w", "$w_0+w_a$", "$10^{9} A_\\mathrm{s}$", "$n_\\mathrm{s}$", 
          "$\\Omega_\\mathrm{c}$", "$H_0$"]
# ------------------------------------------------------------------------------
for i in range(6):
...
plt.savefig(os.environ['ROOTDIR'] + "/projects/roman_real/chains/example_plot_profile2")